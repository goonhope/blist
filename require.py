...
def fetch(url="", hdrs=None, data=None, proxy=None, json=False,g=True,raw=False, tout=5):
    ...
    kw = dict(headers=headers, timeout=tout, proxies=proxy, verify=True)
...
def time_from(t=0, fmt="%Y%m%d %H:%M:%S", g=True):
    """时间相互转换，时间戳与字符串"""
    if isinstance(t, (int, float)):
        flocal = time.gmtime if g else time.localtime
        delta = 0 if not g else 8 * 60 ** 2
